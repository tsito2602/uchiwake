...
- 「取り込み」からレシート画像（JPEG/PNG/WebP、4MB以下）を読み取り、候補を確認・修正してから保存します。AIキーの設定前も手入力とデモ読取が使えます。原本は非公開R2に保存します。
- 「レポート」で費目別の集計と、希望時だけAIコメントを作成します。

## 無料のデモモード

ステージングの「取り込み」と「レポート」では、初期状態で **デモ・無料** が選ばれます。OpenAI APIキーを設定していても、デモではAPIを呼びません。

- 「取り込み」では画像を選ぶか「画像を用意せずサンプルで試す」を押し、スーパー・カフェ・不鮮明の例を選べます。返る日付・店舗・金額・費目は固定サンプルで、画像の内容は解析しません。確認画面で編集して保存すると、デモと記載した支出および選んだ画像がステージングに保存されます。
- 「レポート」では登録済みの費目別合計から定型文を組み立てます。AIの文章生成は行いません。
- 実際のAIを試すときだけ **実際のAI・料金あり** を選びます。キー未設定の場合は選べません。デモモードはステージング専用です。
