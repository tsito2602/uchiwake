...
export type CardStatement = { id: string; due_month: string; title: string; confirmed_total: number; created_at: string };
export type EntryDraft = Pick<CardEntry, 'spent_on' | 'title' | 'category' | 'amount'>;
export type State = { bills: Bill[]; expenses: Expense[]; statements: CardStatement[]; entries: CardEntry[]; ai_enabled: boolean; demo_enabled: boolean; month: string };
