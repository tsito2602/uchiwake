...
import { billKinds, categories, categoryTotals, rentForMonth, summary, type Bill, type BillKind, type Category, type Expense, type EntryDraft, type CardStatement, type SharedCard, type State } from './domain';
...
function App() {
  ...
  async function load() {
    try { setState(await api<State>(`/state?month=${month}`)); setNotice(''); }
    catch(e) { setNotice(String(e instanceof Error?e.message:e)); }
    try { setHistory((await api<{months:{month:string;amount:number}[]}>(`/settlement-history?month=${month}`)).months); }
    catch { setHistory([]); }
  }
  useEffect(()=>{ setState(null);setHistory([]);void load(); },[month]);
  ...
  return <>
    <main className="shell">
      ...
      </>:<Empty text="この月のカード明細はまだありません。" onClick={()=>selectTab('import')} label="カード明細を取り込む"/>}{!!state.expenses.length&&<details className="legacy-details"><summary>以前のレシート記録（精算には含めません）</summary>{state.expenses.map(e=><div className="row" key={e.id}><div className="row-content"><strong>{e.title}</strong><small>{e.spent_on} · {e.category}</small></div><strong className="row-money">{yen(e.amount)}</strong><button className="row-edit" onClick={()=>setEditing({type:'expense',data:e})}>編集</button></div>)}</details>}</>}
      {tab==='import'&&(!draft?<>
    ...
    <FloatingDock tab={tab} onSelect={selectTab} context={dockContext} month={month} onPrevMonth={()=>setMonth(bump(month,-1))} onNextMonth={()=>setMonth(bump(month,1))} add={{label:'追加',options:[{label:'カード明細を取り込む',onClick:()=>selectTab('import')},{label:'今月の家賃を変更',onClick:addBill}]}}/>
