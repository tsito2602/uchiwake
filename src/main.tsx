...
import { ArrowDownLeft, ArrowRight, Camera, Check, ChevronLeft, ChevronRight, CreditCard, Home, Plus, Trash2, X } from 'lucide-react';
import { billKinds, categories, categoryTotals, rentForMonth, summary, type Bill, type Category, type EntryDraft, type SharedCard, type State } from './domain';
...
function App() {
  ...
  const switchDemo=(enabled:boolean)=>{window.sessionStorage.setItem('uchiwake-demo-view',enabled?'1':'0');setOpenCard(null);setCardSettings(null);setDraft(null);setScreenshots([]);setEditing(null);setDemoView(enabled);};
  ...
  const dockContext=(!openCard?.closing&&cardContext)||(!editing?.closing&&billContext)||(!cardSettings?.closing&&settingsContext)||(tab==='import'&&draft?{label:'カード明細の確認',onBack:()=>setDraft(null),actionLabel:busy?'保存中…':'保存して計算',onAction:()=>void saveStatement(),disabled:busy||!canSaveDraft}:undefined);
  return <>
    ...
    <FloatingDock tab={tab} onSelect={selectTab} context={dockContext} panelActive={!!(openCard||editing||cardSettings)} month={month} onPrevMonth={()=>setMonth(bump(month,-1))} onNextMonth={()=>setMonth(bump(month,1))} add={demoView?undefined:{label:'追加',options:[{label:'カード明細を取り込む',onClick:()=>selectTab('import')},{label:'今月の家賃を変更',onClick:()=>addBill()}]}}/>
