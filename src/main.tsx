...
function App() {
  ...
  const dockAdd=!dockContext?{label:'追加',options:[{label:'カード明細を取り込む',onClick:()=>selectTab('import')},{label:'家賃などを追加',onClick:addBill}]}:undefined;
  return <>
    <main className="shell">
      <nav className="desktop-tabs" aria-label="メインメニュー">{dockTabs.map(item=><button key={item.key} aria-current={tab===item.key?'page':undefined} onClick={()=>selectTab(item.key)}><item.icon size={18} strokeWidth={1.7}/>{item.label}</button>)}<button onClick={()=>selectTab('import')}><Plus size={18}/>追加</button></nav>
      <div className="page-top"><h1>{({home:'精算',ledger:'明細',import:'カード明細を追加',report:'記録',settings:'設定'} as const)[tab]}</h1><div className="month-switch"><button aria-label="前月" onClick={()=>setMonth(bump(month,-1))}><ChevronLeft size={18}/></button><span>{monthText(month)}</span><button aria-label="翌月" onClick={()=>setMonth(bump(month,1))}><ChevronRight size={18}/></button></div></div>
      ...
      {tab==='home'&&<>
        <section className="hero"><div className="hero-main"><span className="hero-prefix">精算額 · ひとりあたり</span><div className="hero-money">{yen(totals.perPerson)}</div>{totals.remainder>0&&<p>端数の1円は片方が少なく入金</p>}</div><div className="hero-foot"><span>引落額の合計</span><strong>{yen(totals.total)}</strong></div></section>
        <section className="section settlement-section"><div className="section-head"><h2>内訳</h2><button className="text-action desktop-add-bill" onClick={addBill}><Plus size={16}/> 家賃などを追加</button></div>
          {state.statements.length||state.bills.some(b=>b.kind!=='card')?<div className="settlement-list">{state.statements.map(s=><StatementRow key={s.id} statement={s} onOpen={()=>setTab('ledger')}/>)}{state.bills.filter(b=>b.kind!=='card').map(b=><BillRow key={b.id} bill={b} onEdit={()=>setEditing({type:'bill',data:b})}/>)}</div>:<div className="settlement-list"><button className="settlement-empty-row" onClick={()=>setTab('import')}><CreditCard size={20}/><span>カード明細</span><strong>¥0</strong><ChevronRight size={18}/></button><button className="settlement-empty-row" onClick={addBill}><Home size={20}/><span>家賃など</span><strong>¥0</strong><ChevronRight size={18}/></button></div>}
        ...
        <section className="section record-section"><button className="record-link" onClick={()=>setTab('report')}><span>記録</span><ChevronRight size={20}/></button></section>
      </>}
    ...
    <FloatingDock tab={tab} onSelect={selectTab} add={dockAdd} context={dockContext} month={month} onPrevMonth={()=>setMonth(bump(month,-1))} onNextMonth={()=>setMonth(bump(month,1))}/>
