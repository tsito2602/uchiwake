...
export function ImportReview({draft,cards,settings,busy,checked,onChange,onChecked}:{
  ...
  const unresolved=indexed.filter(({entry})=>isReviewCategory(entry.category,settings));
  const classified=indexed.filter(({entry})=>!isReviewCategory(entry.category,settings));
  ...
  return <div className="import-processing import-review">
    ...
    <div className="import-sorting-list import-review-list" aria-label="仕分け結果">
      {groups.filter(group=>group.rows.length>0).map(group=><section className="import-review-group" key={group.label} aria-label={group.label}>
        {unresolved.length>0&&<div className="import-review-group-heading"><h4>{group.label}<span>{group.rows.length}件</span></h4>{group.pending&&<p role="status">費目を選んでください。要確認が残っている間は保存できません。</p>}</div>}
      {group.rows.map(({entry,index})=><div className="import-review-item" data-expanded={editing===index} key={index} style={{'--import-row-delay':`${Math.min(index,7)*25}ms`} as CSSProperties}>
    ...
    <button className="import-review-add" disabled={busy} onClick={()=>{setEditing(draft.entries.length);onChange({...draft,entries:[...draft.entries,{spent_on:'',title:'',amount:0,category:fallbackCategory(settings)}]});}}><Plus size={16}/> 明細を追加</button>
